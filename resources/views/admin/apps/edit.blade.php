            ...
            <div class="overflow-hidden shadow-sm sm:rounded-lg bg-white">
                <div class="p-6 text-gray-900">
                    <div class="p-8 rounded">
                        <h1 class="font-medium text-3xl">Edit App</h1>
                        <p class="text-gray-600 mt-6">Jika ingin mengupdate app baru, maka isilah beberapa form di
                            bawah ini</p>
                        <form method="POST" action="{{ route('apps.update', $app) }}" enctype="multipart/form-data">
                            ...
                            <div class="mt-8 grid lg:grid-cols-3 gap-4">
                                <div>
                                    <label for="nama"
                                        class="text-sm text-gray-700 block mb-1 font-medium">Nama Aplikasi</label>
                                    <input type="text" name="nama" id="nama"
                                        class="bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full"
                                        placeholder="Nama Aplikasi"  value=" {{ old('nama',$app->nama) }}" />
                                    @error('nama')
                            ...
                            <div class="mt-8">
                                ...
                                <textarea placeholder="Deskripsi Apps" name="deskripsi" id="deskripsi" cols="30"
                                    rows="10"
                                    class="w-full bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700"> {{ old('deskripsi',$app->deskripsi) }}</textarea>
                                @error('deskripsi')
                                <p class="text-red-500 mt-2">{{$message}}</p>
                                @enderror
                            </div>
